...
export const EditPhasesServiceDialog = ({
  ...
  const handleSave = async () => {
    ...
    const newPhases = phases.map((p, i) => ({
      id_fase: p.id_fase || uuidv4(), // Genera uno si no existe
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-xl">
        <DialogHeader>
          <DialogTitle>Editar fases del servicio</DialogTitle>
        </DialogHeader>
            ...
            <div key={index} className="border rounded p-4 space-y-2">
              ...
              <label className="flex items-center gap-2 text-sm">
                <input
                  ...
                  onChange={(e) =>
                    handleChange(index, 'requiere_atencion', e.target.checked)
                  }
                />
                Requiere atención
              </label>
